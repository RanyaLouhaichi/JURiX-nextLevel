# agents/productivity_dashboard_agent.py
# Enhanced ProductivityDashboardAgent with collaborative intelligence
# This agent now collaborates with other agents to provide comprehensive analytics
# and can recognize when additional data or expertise would enhance its analysis
...
class ProductivityDashboardAgent(BaseAgent):
    ...
    def __init__(self, redis_client=None):
        ...
        # Enhanced capabilities including collaboration and forecasting
        self.mental_state.capabilities = [
            AgentCapability.RETRIEVE_DATA,
            AgentCapability.RANK_CONTENT,
            AgentCapability.COORDINATE_AGENTS,  # New: can coordinate with other agents
            AgentCapability.PROVIDE_RECOMMENDATIONS  # Enhanced: can provide analytical recommendations
        ]
        
        # Enhanced obligations for collaborative intelligence
        self.mental_state.obligations.extend([
            ...
            "generate_report",
            "check_for_updates",
            "assess_analytical_depth",      # New: evaluate analysis complexity needs
            "request_additional_data",      # New: request data from other agents
            "provide_predictive_insights",  # New: offer forecasting capabilities
            "collaborate_for_recommendations"  # New: work with recommendation agents
        ])
        
        self.log("Enhanced ProductivityDashboardAgent initialized with collaborative analytics capabilities")
        
        # Start the monitoring thread for collaborative updates
        self.monitoring_thread = threading.Thread(target=self._check_for_updates_loop, daemon=True)
        self.monitoring_thread.start()

    def _check_for_updates(self):
        """Enhanced update checking with collaborative intelligence and proactive analysis"""
        try:
            ...
            if result and result[0]:  # has_changes is True
                ...
                # Fetch the updated tickets with collaborative context
                ...
                if tickets_result:
                    ...
                    # NEW: Assess if this data update suggests collaboration opportunities
                    self._assess_data_update_collaboration_needs(tickets, project_id)
                    
                    # Recalculate metrics with collaborative intelligence
                    metrics = self._analyze_ticket_data(tickets)
                    
                    # Check for significant changes that warrant collaborative analysis
                    if metrics["throughput"] != self.last_throughput:
                        ...
                        # If significant change, request collaborative analysis
                        if change_magnitude > 0.3:  # 30% change
                            self._request_collaborative_analysis(metrics, "significant_change")
                        
                        self.last_throughput = metrics["throughput"]
                        
                        # Generate enhanced dashboard with collaborative insights
                        ...
                        # Store updated dashboard with collaborative metadata
    ...
    def _assess_data_update_collaboration_needs(self, tickets: List[Dict[str, Any]], project_id: str):
        """
        Assess if data updates suggest opportunities for collaborative analysis
        This proactive intelligence helps identify when comprehensive analysis would be valuable
        """
        if not tickets:
            return
        ...
        # If we have substantial new data, suggest comprehensive recommendation analysis
        if ticket_count >= 15:
            self.log("[COLLABORATION OPPORTUNITY] Substantial data update - suggesting recommendation collaboration")
        ...
        # Analyze data characteristics for specialized collaboration needs
    ...
    def _analyze_update_characteristics(self, tickets: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Analyze characteristics of updated data to identify collaboration opportunities"""
        characteristics = {
        ...
        for ticket in tickets:
            ...
            # Complexity analysis (based on description length and changelog activity)
        ...
        # Suggest knowledge base analysis if we have complex, resolved tickets
    ...
    def _check_for_updates_loop(self):
        ...
        while True:
            try:
                ...
                time.sleep(10)  # Reduced for demo purposes
            except Exception as e:
    ...
    def _perceive(self, input_data: Dict[str, Any]) -> None:
        ...
        # Store enhanced beliefs about the analysis request
        ...
        # NEW: Assess collaboration needs for this analysis request
    ...
    def _assess_analysis_collaboration_needs(self, input_data: Dict[str, Any]) -> None:
        """
        Assess if this productivity analysis would benefit from collaboration
        This intelligence helps the agent understand when it should coordinate with others
        """
        tickets = input_data.get("tickets", [])
        ...
        # If we have limited ticket data, request more comprehensive data
        ...
        # If analysis reveals patterns that could generate recommendations, suggest collaboration
        if len(tickets) >= 10:
    ...
    def _analysis_suggests_recommendations(self, metrics: Dict[str, Any]) -> bool:
        """Determine if analysis results suggest recommendation collaboration would be valuable"""
        # Look for patterns that suggest actionable recommendations
        bottlenecks = metrics.get("bottlenecks", {})
        throughput = metrics.get("throughput", 0)
        workload = metrics.get("workload", {})
        
        # Suggest recommendations if we detect issues or opportunities
    ...
    def _analyze_ticket_data(self, tickets: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Enhanced ticket analysis with collaborative intelligence and deeper insights"""
        ...
        try:
            # Core analysis with enhanced intelligence
            assignee_counts = {}
            status_counts = {}
            done_tickets = []
            cycle_times = []
            ...
            metrics = {
                ...
                "data_quality_score": len(tickets) / 20.0  # Normalize to 0-1 scale, optimal around 20 tickets
            }
    ...
    def _calculate_enhanced_cycle_time(self, ticket: Dict[str, Any]) -> float:
        """Calculate more sophisticated cycle time with collaborative intelligence"""
        try:
            changelog = ticket.get("changelog", {}).get("histories", [])
            if not changelog:
                return 0.0
            
            # Find actual work periods (In Progress to Done)
            work_start = None
            work_end = None
            ...
            if work_start and work_end:
                ...
                return max(cycle_days, 0)  # Ensure non-negative
    ...
    def _calculate_analysis_confidence(self, tickets: List[Dict[str, Any]]) -> float:
        """Calculate confidence in analysis based on data quality and completeness"""
        if not tickets:
            return 0.0
        ...
        sample_size_score = min(len(tickets) / 20.0, 1.0)  # Optimal around 20+ tickets
    ...
    def _generate_collaborative_insights(self, assignee_counts: Dict[str, int], 
        ...
        """Generate intelligent insights that can inform collaborative recommendations"""
        insights = {
        ...
        if assignee_counts:
            ...
            if balance_ratio < 0.5:  # Significant imbalance
        ...
        if status_counts:
            ...
            if total_active > 0:
                ...
                if efficiency_score < 0.4:  # Low completion rate
        ...
        if cycle_times:
            ...
            insights["predictive_indicators"]["cycle_time_trend"] = "stable"  # Could be enhanced with historical data
        ...
        if resolution_patterns:
            recent_weeks = sorted(resolution_patterns.keys())[-4:]  # Last 4 weeks
    ...
    def _get_collaborative_recommendations(self, metrics: Dict[str, Any]) -> List[str]:
        """Generate or retrieve recommendations through collaboration"""
        ...
        # If we have collaboration opportunities identified, generate enhanced recommendations
        ...
        # Add intelligent recommendations based on collaborative insights
        ...
        # If we have no recommendations, generate intelligent defaults based on metrics
    ...
    def _generate_intelligent_default_recommendations(self, metrics: Dict[str, Any]) -> List[str]:
        """Generate intelligent default recommendations based on collaborative analysis"""
        recommendations = []
    ...
    def _request_collaborative_analysis(self, metrics: Dict[str, Any], trigger: str):
        """Request collaborative analysis when significant changes are detected"""
        self.log(f"[COLLABORATION REQUEST] Requesting collaborative analysis due to: {trigger}")
        
        # Request recommendation collaboration for significant changes
        self.mental_state.request_collaboration(
            agent_type="recommendation_agent",
            reasoning_type="strategic_reasoning",
            context={
    ...
    def _create_visualization_data(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        """Enhanced visualization creation with collaborative insights"""
        try:
            charts = []
            tables = []
            ...
            # NEW: Collaborative insights visualization
            collaborative_insights = metrics.get("collaborative_insights", {})
                    ...
                    "data": {
                        ...
                        "datasets": [{
                            ...
                            "data": [
                                ...
                                min(100 - (metrics.get("cycle_time", 0) * 10), 100),  # Inverse for cycle time
                                min(metrics.get("throughput", 0) * 10, 100)  # Scale throughput
                            ],
            ...
            # NEW: Recommendations table if available
            recommendations = self._get_collaborative_recommendations(metrics)
            if recommendations:
                rec_table = {
    ...
    def _generate_smart_colors(self, count: int) -> List[str]:
        """Generate intelligent colors based on workload distribution"""
        colors = ["#4CAF50", "#FF9800", "#F44336", "#9C27B0", "#2196F3"]  # Green, Orange, Red, Purple, Blue
        return [colors[i % len(colors)] for i in range(count)]

    def _generate_bottleneck_colors(self, bottlenecks: Dict[str, int]) -> List[str]:
        """Generate colors that highlight bottlenecks (red for high counts)"""
        max_count = max(bottlenecks.values()) if bottlenecks else 1
        colors = []
        
        for count in bottlenecks.values():
            if count >= max_count * 0.8:  # High bottleneck
                colors.append("#F44336")  # Red
            elif count >= max_count * 0.5:  # Medium bottleneck
    ...
    def _generate_report(self, metrics: Dict[str, Any], recommendations: List[str]) -> str:
        """Enhanced report generation with collaborative intelligence and actionable insights"""
        try:
            today = datetime.now().strftime("%Y-%m-%d")
            
            # Generate intelligent insights based on collaborative analysis
            insights = []
            collaborative_insights = metrics.get("collaborative_insights", {})
            ...
            # Analysis confidence and data quality
            analysis_confidence = metrics.get("analysis_confidence", 0.5)
            if analysis_confidence < 0.6:
...
Generated at {datetime.now().strftime('%H:%M:%S')}"""
    ...
    def _act(self) -> Dict[str, Any]:
        """Enhanced action method with comprehensive collaborative intelligence"""
        try:
            ...
            # Store analysis request as experience
            ...
            # Perform enhanced analysis with collaborative intelligence
            metrics = self._analyze_ticket_data(tickets)
            self.last_throughput = metrics["throughput"]
            ...
            # Get or generate collaborative recommendations
            enhanced_recommendations = self._get_collaborative_recommendations(metrics)
            
            # Generate comprehensive report
            report = self._generate_report(metrics, enhanced_recommendations)
            
            # Store successful analysis as experience
            if hasattr(self.mental_state, 'add_experience'):
                self.mental_state.add_experience(
    ...
    def _rethink(self, action_result: Dict[str, Any]) -> None:
        """Enhanced reflection with collaborative performance analysis"""
        ...
        # Analyze collaborative effectiveness
        ...
        # Enhanced reflection with collaborative intelligence
        reflection = {
        ...
        # Update competencies based on collaborative performance
        if status == "success":
    ...
    def get_collaborative_performance_metrics(self) -> Dict[str, Any]:
        """Get enhanced performance metrics including collaborative intelligence indicators"""
        try:
            ...
            # Get collaboration effectiveness from reflections
    ...
    def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """Main entry point for collaborative productivity analysis"""
